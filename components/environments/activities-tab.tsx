...
import { Play, Loader2, Clock, CheckCircle2, XCircle, AlertCircle, RefreshCw, ListChecks, ScrollText, ChevronDown, ChevronUp, Zap } from "lucide-react"
import { cn } from "@/lib/utils"
import { templatesApi } from "@/lib/api/templates"
import { environmentsApi } from "@/lib/api/environments"
import type { Activity, ActivityRun, Deployment, Environment, ProvisionedResource } from "@/lib/api/types"
...
function statusIcon(status: string) {
  ...
  if (s === "RUNNING" || s === "QUEUED" || s === "INITIALIZING") return <Loader2 className="h-3.5 w-3.5 animate-spin text-primary" />
...
interface ActivityCardProps {
  activity: Activity
  runs: ActivityRun[]
  onTrigger: (activityId: string) => Promise<void>
  onViewLogs: (run: ActivityRun) => void
  triggering: boolean
  disabled: boolean
}

function ActivityCard({ activity, runs, onTrigger, onViewLogs, triggering, disabled }: ActivityCardProps) {
    ...
    <div className="flex flex-col gap-3 rounded-lg border border-border p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Button
            ...
            onClick={() => onTrigger(String(activity.id))}
      ...
      {yamlOpen && (
        <div className="border-t border-border pt-3">
          <PlaybookYamlViewer yaml={activity.playbook_yaml} />
        </div>
      )}
...
export function ActivitiesTab({ projectId, environmentId, templateId, versionId, deployments, environment, resourcesByDeployment }: ActivitiesTabProps) {
  ...
  const pollRuns = useCallback(async () => {
    const runsData = await environmentsApi.listPlaybookRuns(projectId, environmentId).catch(() => null)
  ...
  const handleTrigger = async (activityId: string) => {
    setTriggeringId(activityId)
    try {
      const newRun = await environmentsApi.triggerPlaybookRun(projectId, environmentId, activityId)
  ...
  const handleViewLogs = (run: ActivityRun) => {
    const activity = activities.find((a) => String(a.id) === String(run.playbook_id ?? run.activity_id))
    setLogResource({
      type:          "activity",
      projectId,
      environmentId,
      runId:         String(run.id),
      title:         `Logs — ${run.playbook_name ?? run.activity_name ?? activity?.name ?? "Playbook"}`,
    })
  }

  const runsByActivity = (activityId: string) =>
    runs
      .filter((r) => String(r.playbook_id ?? r.activity_id) === activityId)
    ...
    <div className="flex flex-col gap-6">
      ...
      {!hasRunningDeployment && (
        <div className="flex items-start gap-2 rounded-lg border border-amber-400/40 bg-amber-50 dark:bg-amber-950/20 px-4 py-3 text-xs text-amber-700 dark:text-amber-400">
          ...
          <span>
            <strong>Playbooks are disabled.</strong> At least one deployment must be in the{" "}
            <strong>Running</strong> state before you can execute a playbook.
          </span>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-dashed border-border p-6 text-center">
          ...
          <p className="text-sm text-muted-foreground">No on-demand playbooks configured for this template.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          {activities.map((a) => (
            <ActivityCard
              key={a.id}
              activity={a}
              runs={runsByActivity(String(a.id))}
              onTrigger={handleTrigger}
              onViewLogs={handleViewLogs}
              triggering={triggeringId === String(a.id)}
